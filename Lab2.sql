...
SELECT ACTNO,AMOUNT,CNAME FROM DEPOSITE WHERE ACTNO=106;
...
SELECT CNAME,BNAME,AMOUNT FROM DEPOSITE WHERE AMOUNT>4000 AND ACTNO<105;

---Find all borrowers whose amount is greater than equals to 3000 & less than equals to 8000 from borrowtable. (AND & BETWEEN)
	---AND
SELECT BNAME,AMOUNT FROM BORROW WHERE AMOUNT>3000 AND AMOUNT<8000;
	---BETWEEN
SELECT BNAME,AMOUNT FROM BORROW WHERE AMOUNT BETWEEN 3000 AND 8000;

---Find all depositors who do not belongs to �ANDHERI� branch from deposit table.
SELECT BNAME FROM DEPOSITE WHERE BNAME!='ANDHERI';
...
SELECT TOP 10 PERCENT LOANNO,AMOUNT FROM BORROW;
...
SELECT *,AMOUNT+AMOUNT*0.1 AS ADD_AMOUNT FROM BORROW;
